...
def test_jvp_jvp() raises -> None:
    ...
    def hvp_fn_raw(args: List[nabla.Array]) raises -> List[nabla.Array]:
        var primals = List(args[: len(args) // 2])
        var tangents = List(args[len(args) // 2 :])
        # return nabla.jvp(grad_fn, primals, tangents)[1]
        return nabla.jvp(nabla.vmap(grad_fn, [nabla.none, 0]), primals, tangents)[1].copy()  # JVP on gradient computes H(x)·v
