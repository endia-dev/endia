...
def test_vmap_jvp() raises:
    ...
    def jvp_vmapped(args: List[nabla.Array]) raises -> List[nabla.Array]:
        var primals = List(args[: len(args) // 2])
        var tangents = List(args[len(args) // 2 :])
        # print("Start VMAP")
        var foo_vmap = nabla.vmap(foo)
        # print("End VMAP")
        return nabla.jvp(foo_vmap, primals, tangents)[1].copy()  # This extracts f'(x)
