# ===----------------------------------------------------------------------=== #
# Nabla 2025
#
# Licensed under the Apache License v2.0 with LLVM Exceptions:
# https://llvm.org/LICENSE.txt
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ===----------------------------------------------------------------------=== #

import nabla


def test_jacfwd_vmap() raises:
    var x = nabla.arange((3, 2, 3)) + 2
    var y = nabla.arange((3, 2, 3)) + 3

    def foo(args: List[nabla.Array]) raises -> List[nabla.Array]:
        var x = args[0]
        var y = args[1]
        var res: List[nabla.Array] = [nabla.sin(x) + x**2 + y**2, nabla.cos(y) + y * x]
        return res.copy()

    def vmapped_foo(args: List[nabla.Array]) raises -> List[nabla.Array]:
        var res = nabla.vmap(foo)(args)
        return res.copy()

    print("Creating jacobian")
    var jacobian = nabla.jacfwd(vmapped_foo)
    print("Computed jacobian")
    jacobian_result = jacobian([x, y])
    print(nabla.xpr(jacobian)([x, y]))

    print("\nJacobian 1:")
    print(jacobian_result[0])
    print("\nJacobian 2:")
    print(jacobian_result[1])
    print("\nJacobian 3:")
    print(jacobian_result[2])
    print("\nJacobian 4:")
    print(jacobian_result[3])
